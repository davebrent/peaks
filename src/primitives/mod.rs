...
impl Aabb {
    ...
    /// Returns the distance from the furthest corner of the box
    pub fn min_corner_dist(&self, origin: Vec3) -> f64 {
        self.corners()
            .iter()
            .map(|corner| Vec3::distance(*corner, origin))
            .fold(INFINITY, |min, f| if f < min { f } else { min })
    }

    /// Returns the distance from the closest corner of the box
    pub fn max_corner_dist(&self, origin: Vec3) -> f64 {
        self.corners()
            .iter()
            .map(|corner| Vec3::distance(*corner, origin))
            .fold(-INFINITY, |max, f| if f > max { f } else { max })
    }

    fn corners(&self) -> [Vec3; 8] {
        [
            // Top
            Vec3::new(self.min.x, self.max.y, self.min.z), // ne
            Vec3::new(self.max.x, self.max.y, self.min.z), // nw
            Vec3::new(self.max.x, self.max.y, self.max.z), // sw
            Vec3::new(self.min.x, self.max.y, self.max.z), // se
            // Bottom
